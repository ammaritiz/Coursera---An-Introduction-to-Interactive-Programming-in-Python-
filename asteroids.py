...
class Sprite:
    ...
    def draw(self, canvas):
        if not self.animated:
            ...
            explosion_pos = explosion_info.get_center()
            explosion_size = explosion_info.get_size()
            canvas.draw_image(explosion_image, [ self.age * explosion_size[0] + explosion_pos[0], explosion_pos[1]],explosion_info.get_size(), self.pos,self.image_size)
...
def group_group_collide(group1, group2):
    ...
    for sprite in set(group1):
        collided = False
        collided = group_collide(group2, sprite)
        if collided:
...
def rock_spawner():
    global a_rock, lives, my_ship, started
